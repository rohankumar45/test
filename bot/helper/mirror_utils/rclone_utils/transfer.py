...
from aiofiles.os import path as aiopath, listdir, mkdir
...
from bot import config_dict, LOGGER, GLOBAL_EXTENSION_FILTER
...
class RcloneTransferHelper:
    ...
    async def __start_download(self, cmd, remote_type):
        ...
        if return_code == 0:
            ...
            await self.__listener.onDownloadError(error, ename=self.name)
    ...
    async def download(self, remote, rc_path, config_path, path):
        ...
        try:
            ...
            await self.__listener.onDownloadError(err, ename=self.name)
            return
        remote_type = remote_opts['type']
        if remote_type == 'drive' and config_dict['USE_SERVICE_ACCOUNTS'] and config_path == 'rclone.conf' and await aiopath.isdir('accounts'):
        ...
        if return_code == -9:
            ...
            if self.__sa_number != 0 and remote_type == 'drive' and 'RATE_LIMIT_EXCEEDED' in error and config_dict['USE_SERVICE_ACCOUNTS']:
                if self.__sa_count < self.__sa_number:
                    ...
                    LOGGER.info(f'Reached maximum number of service accounts switching, which is {self.__sa_count}')
            await self.__listener.onUploadError(error, self.name)
            return False
        else:
            return True
    ...
    async def upload(self, path, size):
        ...
        if await aiopath.isdir(path):
            ...
            if path.lower().endswith(tuple(GLOBAL_EXTENSION_FILTER)):
                await self.__listener.onUploadError(f'This file extension is excluded by extension filter ({", ".join(GLOBAL_EXTENSION_FILTER[2:])})!', self.name)
                return
        ...
        try:
            ...
            await self.__listener.onUploadError(err, self.name)
            return
        remote_type = remote_opts['type']
        fremote = oremote
    ...
    async def clone(self, config_path, src_remote, src_path, destination, rcflags, mime_type):
        ...
        if return_code == -9:
            ...
            error = (await self.__proc.stderr.read()).decode().strip()
            LOGGER.error(error)
            await self.__listener.onUploadError(error, f'{src_remote}:{src_path}', True)
            return None, None
        else:
            if dst_remote_opt == 'drive':
                ...
                if code == 0:
                    ...
                    LOGGER.error(f'while getting link. Path: {destination} | Stderr: {err}')
                    await self.__listener.onUploadError(err, f'{src_remote}:{src_path}', True)
                    return None, None

    async def gclone(self, config_path, drive_id, destination, mime_type):
        def_remote = destination.split(':', 1)[0]
        if mime_type == 'Folder':
            destination = f'{destination}/{self.name}'
        ...
        if return_code == -9:
            ...
            error = (await self.__proc.stderr.read()).decode().strip()
            LOGGER.error(error)
            await self.__listener.onUploadError(error, self.name, True)
            return None, None
        else:
            dst_remote, dst_path = destination.split(':', 1)
            link, destination = await self.__get_gdrive_link(config_path, dst_remote, dst_path, mime_type)
    ...
    def __getUpdatedCommand(config_path, source, destination, rcflags, method):
        ...
        cmd = ['./gclone', method, '--fast-list', '--config', config_path, '-P', source, destination, '--exclude', ext, '--ignore-case', '--low-level-retries', '1', '-M', '--log-file', 'rlog.txt', '--log-level', 'DEBUG']
        if rcflags:
