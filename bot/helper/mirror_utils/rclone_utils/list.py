...
class RcloneList:
    ...
    async def get_path_buttons(self):
        ...
        msg += f'<i>Timeout: {get_readable_time(TIMEOUT-(time()-self.__time))}</i>'
        if not self.is_cancelled:
            await editMessage(msg, self.__message, buttons.build_menu(f_cols=2))

    async def get_path(self, itype=''):
        await editMessage(f'<i>Listing file(s), please wait...</i>', self.__message)
        if itype:
