...
class YoutubeDLHelper:
    ...
    async def add_download(self, link, path, name, qual, playlist, options):
        ...
        if self.is_playlist:
            ...
            self.opts['outtmpl'] = {'default': f"{path}/{base_name}/{self.name}", 'thumbnail': f"{path}/yt-dlp-thumb/{base_name}.%(ext)s"}
        else:
        ...
        if file:
            ...
            return
        msgerr = None
        arch = any([self.__listener.compress, self.__listener.isLeech])
        ...
        elif zuzdl and self.__listener.compress and self.__size >= zuzdl * 1024**3:
