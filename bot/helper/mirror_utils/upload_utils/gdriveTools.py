...
class GoogleDriveHelper:
    ...
    def deletefile(self, link: str):
        ...
        try:
            ...
            if any(x in str(err) for x in ['File not found', 'insufficientFilePermissions']):
            ...
            return msg or 'File not found or insufficient file rermissions!'

    def upload(self, file_name: str, size: int, gd_id: str):
        ...
        try:
            if ospath.isfile(item_path):
                ...
                link = self.__upload_file(item_path, file_name, mime_type, gd_id, is_dir=False)
                if self.__is_cancelled:
                    return
                if link is None:
                    raise Exception('Upload has been manually cancelled!')
                LOGGER.info(f"Uploaded to GDrive: {item_path}")
            else:
                mime_type = 'Folder'
                dir_id = self.__create_directory(ospath.basename(ospath.abspath(file_name)), gd_id)
                ...
                if self.__is_cancelled:
                    return
                LOGGER.info(f"Uploaded to GDrive: {file_name}")
        except Exception as err:
            if isinstance(err, RetryError):
                LOGGER.info(f"Total Attempts: {err.last_attempt.attempt_number}")
                err = err.last_attempt.exception()
            err = str(err).replace('>', '').replace('<', '')
            async_to_sync(self.__listener.onUploadError, err, file_name)
    ...
    def __upload_file(self, file_path, file_name, mime_type, dest_id, is_dir=True):
        ...
        if dest_id is not None:
            file_metadata['parents'] = [dest_id]
        if async_to_sync(get_path_size, file_path) == 0:
    ...
    def clone(self, link: str, name: str, sharer_link=None):
        ...
        try:
            ...
            if mime_type == self.__G_DRIVE_DIR_MIME_TYPE:
                dir_id = self.__create_directory(meta.get('name'), self.__listener.multiId[1])
                ...
                file = self.__copyFile(meta.get('id'), self.__listener.multiId[1], name)
    ...
    def __copyFile(self, file_id, dest_id, new_name):
        body = {"parents": [dest_id], "name": new_name}
        try:
    ...
    def drive_list(self, fileName, stopDup=False, noMulti=False, isRecursive=True, itemType="", style='html'):
        msg = ''
                ...
                if mime_type == "application/vnd.google-apps.folder":
                    ...
                    if index_url:
                        ...
                        if config_dict['VIEW_LINK']:
                            urlv = short_url(f'{index_url}/{url_path}?a=view')
                            if style in ['tele', 'graph']:
    ...
    def count(self, link):
        try:
            ...
            return "Google Drive ID could not be found in the provided link", None, None, None,
        msg = ""
        LOGGER.info(f"File ID: {file_id}")
        try:
    ...
    def get_id(self, link):
        try:
            return self.__getIdFromUrl(link)
    ...
    def download(self, link):
        ...
        try:
            ...
            if "downloadQuotaExceeded" in err:
                ...
                err = 'File not found!'
            async_to_sync(self.__listener.onDownloadError, err, ename=self.name)
    ...
    async def cancel_download(self):
        ...
        if self.__is_downloading:
            ...
            await self.__listener.onUploadError('Your clone has been stopped and cloned data has been deleted!', self.name, True)
        elif self.__is_uploading:
            LOGGER.info(f"Cancelling Upload: {self.name}")
            await self.__listener.onUploadError('Your upload has been stopped and uploaded data has been deleted!', self.name)