...
from bot import config_dict, bot_dict, LOGGER, DEFAULT_SPLIT_SIZE
from bot.helper.ext_utils.bot_utils import cmd_exec, sync_to_async, is_premium_user
from bot.helper.ext_utils.fs_utils import ARCH_EXT, get_mime_type, get_path_size, clean_target
...
async def take_ss(video_file, duration):
    ...
    if duration == 0:
        duration = 3
    duration = duration // 2
    cmd = ['ffmpeg', '-hide_banner', '-loglevel', 'error', '-ss', str(duration), '-i', video_file, '-vf', 'thumbnail', '-frames:v', '1', des_dir]
...
async def split_file(path, size, file_, dirpath, split_size, listener, start_time=0, i=1, inLoop=False, multi_streams=True):
    if listener.suproc == 'cancelled' or listener.suproc is not None and listener.suproc.returncode == -9:
        return False
    MAX_SPLIT_SIZE = bot_dict['MAX_SPLIT_SIZE']
    ...
    if config_dict['PREMIUM_MODE']:
        if not is_premium_user(listener.user_id):
    ...
    if (await get_document_type(path))[0]:
        ...
        while i <= parts or start_time < duration - 4:
            ...
            cmd = ['ffmpeg', '-hide_banner', '-loglevel', 'error', '-ss', str(start_time), '-i', path, '-fs', str(split_size),
