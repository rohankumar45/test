...
async def get_button(from_user, mid, data=None, isRecursive=False):
    ...
    if not data:
        ...
        text = f"{from_user.mention}, Choose Option to Search <b>{content_dict[mid]['content'].key.title()}</b>."
    else:
...
@new_task
async def list_buttons(_, message: Message):
...
@new_task
async def select_type(_, query: CallbackQuery):
    ...
    try:
        ...
        try:
            ...
            pass
    tele: TeleContent = content_dict.get(mid, {}).get('content')
    if not tele and data[2] != 'close':
        ...
        content_dict[mid]['recursive'] = isRecursive
...
async def _list_drive(key: str, message: Message, item_type: str, style: str):
    ...
    recursive = content_dict[omsg.id]['recursive']
    ...
    if count:
        ...
        if style == 'tele':
            tele: TeleContent = content_dict[omsg.id]['content']
